...
int main(int argc, char *argv[]) {
  ...
  if (app.arguments().size() > 1) {
    QObject::connect(
      app_context.authorization_service,
      &AuthorizationService::authorized,
      &app,
      [&app, &app_context]() {
        CLI cli(app_context, app_context.yandex_api);
      });
    ...
    app_context.authorization_service->AttemptLocalAuthorization();
  ...
  const auto scenarios_model = new ScenariosModel(app_context.yandex_api, &app);
  const auto devices_model = new DevicesModel(app_context.yandex_api, &app);
  const auto rooms_model = new RoomsModel(app_context.yandex_api, &app);
  const auto device_controller = new DeviceController(app_context.yandex_api, &app);
  const auto households_model = new HouseholdsModel(app_context.yandex_api, &app);
